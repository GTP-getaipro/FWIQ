...
class N8nCredentialManager {
  ...
  async createGmailCredential(businessId, oauthData, businessDomain) {
    ...
    try {
      const response = await axios.post(`${this.n8nApiUrl}/api/v1/credentials`, credentialPayload, {
        headers: {
      ...
      await this.storeBusinessCredential(businessId, credential.id, credentialName);
      
      console.log(`✅ Created n8n credential: ${credentialName} (ID: ${credential.id})`);
      return credential;

    } catch (error) {
      console.error('❌ Failed to create n8n credential:', error.response?.data || error.message);
      throw new Error(`Failed to create n8n credential: ${error.message}`);
    }
  }
